...
function NavButton({ item, active, badge, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 px-5 py-2.5 text-sm font-medium transition-all border-l-4 w-full ${
        active
          ? activeStyles
...
export default function Sidebar({
  ...
  return (
    <>
      {/* Spacer — evita que el contenido quede tapado por el sidebar fijo */}
      <div className="w-56 flex-shrink-0" aria-hidden="true" />

      <aside className="fixed top-0 left-0 h-screen w-56 border-r border-[var(--c-border)] bg-[var(--c-surface)] flex flex-col z-30">
        {/* ── Logo ── */}
